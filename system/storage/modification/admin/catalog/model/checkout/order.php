...
class ModelCheckoutOrder extends Model {
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		$order_info = $this->getOrder($order_id);
                ...
                //Shoputils.Mail.Order.Status Extension Begin
                $this->load->model('extension/module/shoputils_mail_order_status');
                //Shoputils.Mail.Order.Status Extension End
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
                ...
                //if (!$this->model_extension_module_shoputils_mail_order_status->confirmOrder((int)$order_id, (int)$order_status_id, $comment)) {
                if (!$this->model_extension_module_shoputils_mail_order_status->confirmOrder(array('order_id' => $order_id, 'order_status_id' => $order_status_id, 'admin_comment' => $comment))) {
                //Shoputils.Mail.Order.Status Extension End
                ...
                //Shoputils.Mail.Order.Status Extension Begin
                //Send Mail to Admin
                if (!$this->model_extension_module_shoputils_mail_order_status->confirmOrderAdmin(array('order_id' => $order_id, 'order_status_id' => $order_status_id, 'admin_comment' => $comment))) {
                    $config_order_mail = in_array('order', (array)$this->config->get('config_mail_alert'));
                } else {
                    $config_order_mail = false;
                }
                if ($config_order_mail) {
                ...
                if (!empty($customInfo['field20'])) {
                    $parts = explode('.', $customInfo['field20']);
                    $parts = array_slice($parts, 0, count($parts) - 1);

                    $tmp_file = DIR_DOWNLOAD . implode('.', $parts);

                    copy(DIR_DOWNLOAD . $customInfo['field20'], $tmp_file);
			...
			// If order status is not 0 then send update text email
                ...
                //Update order
                if ($order_info['order_status_id'] && $order_status_id) {
                    //Совместимость с отсроченной оплатой от Ашапа
                    $mail_comment = $comment;
                    if (isset($merchant_url)) {
                        $mail_comment .= "\n" . 'Перейдите по ссылке для оплаты онлайн' . "\n" . strip_tags(html_entity_decode($merchant_url, ENT_QUOTES, 'UTF-8'));
                    }
                    if (isset($cash_pay_url)) {
                        $mail_comment .= "\n" . 'Перейдите по ссылке для оплаты онлайн' . "\n" . strip_tags(html_entity_decode($cash_pay_url, ENT_QUOTES, 'UTF-8'));
                    }
                    if ($this->config->get('cash_pay_order_on_status_id') == $order_status_id & $order_info['payment_code'] == "cash_pay") {
                        if (isset($file_location)) {$attachment = $file_location;}
                    } 
                    //Send Mail to Admin
                    $trackcode = isset($data['trackcode']) ? $data['trackcode'] : '';
                    //$this->model_extension_module_shoputils_mail_order_status->updateOrderAdmin((int)$order_id, (int)$order_status_id, $mail_comment, $trackcode, isset($attachment) ? $attachment : '');
                    $this->model_extension_module_shoputils_mail_order_status->updateOrderAdmin(array('order_id' => $order_id, 'order_status_id' => $order_status_id, 'admin_comment' => $mail_comment, 'trackcode' => $trackcode, 'attachment' => isset($attachment) ? $attachment : ''));
                    //Send Mail to Customer
                    //$shoputils_sendmail_result = $this->model_extension_module_shoputils_mail_order_status->updateOrder((int)$order_id, (int)$order_status_id, $mail_comment, $notify, true, $trackcode, isset($attachment) ? $attachment : '');
                    $shoputils_sendmail_result = $this->model_extension_module_shoputils_mail_order_status->updateOrder(array('order_id' => $order_id, 'order_status_id' => $order_status_id, 'admin_comment' => $mail_comment, 'admin_notify' => $notify, 'send' => true, 'trackcode' => $trackcode, 'attachment' => isset($attachment) ? $attachment : ''));
