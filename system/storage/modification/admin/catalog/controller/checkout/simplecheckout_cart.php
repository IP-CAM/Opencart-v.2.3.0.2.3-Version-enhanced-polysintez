...
*/
        ...
        $this->_templateData['hide'] = $this->simplecheckout->isBlockHidden('cart');
				...
				$heading_title = ($image . $this->language->get('text_items'));
        ...
        if ($currentTheme == 'shoppica' || $currentTheme == 'shoppica2') {
            ...
            $this->_templateData['cart_total'] = sprintf($heading_title, $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0), $this->simplecheckout->formatCurrency($total));
        }
