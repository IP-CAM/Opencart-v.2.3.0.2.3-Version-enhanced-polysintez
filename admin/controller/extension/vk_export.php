...
class ControllerExtensionVKExport extends Controller {
    ...
    public function market_export() {
        ...
        if (isset($this->request->post['selected']) && $this->validateVkExport()) {
            ...
            foreach ($this->request->post['selected'] as $key => $product_id) {
                ...
                $this->errorMessage = '';
                if (!$this->makeLoadMarket($vkApi, $product_id, $p, $category_id, $vk_market_category_id)) {
                    $error = true;
                }
                if ($errmsg = $this->errorMessage) {
                    $report[] = $errmsg;
    ...
    private function makeLoad($vkApi, $product_id, $category_id, $p, $mode) {
        ...
        foreach ($images_in as $img) {
            if ($this->config->get('vk_export_image_mode') == 1) {
                ...
                $img = $this->model_tool_image->resize($img, $this->config->get('theme_default_image_popup_width'), $this->config->get('theme_default_image_popup_height'));
    ...
    private function makeLoadMarket($vkApi, $product_id, $p, $category_id, $vk_market_category_id) {
        ...
        foreach ($images_in as $img) {
            if ($this->config->get('vk_export_image_mode') == 1) {
                ...
                // изменение размера фото
                $_img = $this->model_tool_image->resize($img, $this->config->get('theme_default_image_popup_width'), $this->config->get('theme_default_image_popup_height'));
                $_img = DIR_IMAGE . str_replace(($this->config->get('config_secure') ? HTTPS_CATALOG : HTTP_CATALOG), '', $_img);
        ...
        // описание
        $message = str_replace(array_keys($vars), array_values($vars), $this->config->get('vk_export_market_product_desc_tpl'));
        $message = strip_tags(html_entity_decode($message, ENT_COMPAT, 'UTF-8'));
        $message = str_replace(array('&nbsp;'), array(' '), $message);
        $user_replacements_keys = htmlspecialchars_decode($this->config->get('vk_export_user_replacements_keys'));
        $user_replacements_keys = explode("\n", $user_replacements_keys);
                    ...
                    if (!$album_id) {
                        ...
                        try {
                            ...
                            if ($img_width < 1280 || $img_height < 720) {
                                ...
                                $img = DIR_IMAGE . str_replace(($this->config->get('config_secure') ? HTTPS_CATALOG : HTTP_CATALOG), '', $img);
                                $img = str_replace('/image/image/', '/image/', $img);
                            }
    ...
    public function send_captcha() {
      ...
      private function getList() {                
        ...
        if (isset($this->request->get['filter_manufacturer_id']) && $this->request->get['filter_manufacturer_id']) {
        ...
        foreach ($_vk_market_cats as $cat) {
        ...
        $product_total = $this->model_catalog_vk_export->getFoundRows($params);
    ...
    public function market_album_add() {
        ...
        // категории товаров ВК
        $data['vk_market_categories'] = json_decode($this->config->get('vk_export_market_cats'));
