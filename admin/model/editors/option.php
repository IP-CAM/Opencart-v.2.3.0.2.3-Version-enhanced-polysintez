...
class ModelEditorsOption extends Model {
	...
	public function copyOption($option_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "option o WHERE o.option_id = '" . (int)$option_id . "'");
		...
		if ($query->num_rows) {
			...
			$data['option_description'] = $this->getOptionDescriptions($option_id);
			$data['option_value'] = $this->getOptionValueDescriptions($option_id);

			$this->addOption($data);

			return $this->new_option_id;
		}
	}
