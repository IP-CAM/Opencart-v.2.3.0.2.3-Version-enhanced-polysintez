...
class ModelEditorsAttributeGroup extends Model {
	...
	public function copyAttributeGroup($attribute_group_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "attribute_group ag WHERE ag.attribute_group_id = '" . (int)$attribute_group_id . "'");
		...
		if ($query->num_rows) {
			...
			$data['attribute_group_description'] = $this->getAttributeGroupDescriptions($attribute_group_id);

			$this->addAttributeGroup($data);

			return $this->new_attribute_group_id;
		}
	}
