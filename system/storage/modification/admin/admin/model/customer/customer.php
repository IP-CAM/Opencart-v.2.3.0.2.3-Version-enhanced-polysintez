	...
	public function addTransaction($customer_id, $description = '', $amount = '', $order_id = 0) {
		...
		if ($customer_info) {
			...
			$message .= sprintf($this->language->get('text_transaction_total'), $this->currency->format($this->getTransactionTotal($customer_id), $this->session->data['currency']));
                ...
                //Send Mail to Admin
                $this->model_extension_module_shoputils_mail_order_status->customerTransactionAdmin(array('customer_id' => $customer_id, 'description' => $description, 'amount' => $this->currency->format($amount, $this->config->get('config_currency')), 'total' => $this->currency->format($this->getTransactionTotal($customer_id), $this->config->get('config_currency')), 'order_id' => $order_id));
                //Send Mail to Affiliate
                if (!$this->model_extension_module_shoputils_mail_order_status->customerTransaction(array('customer_id' => $customer_id, 'description' => $description, 'amount' => $this->currency->format($amount, $this->config->get('config_currency')), 'total' => $this->currency->format($this->getTransactionTotal($customer_id), $this->config->get('config_currency')), 'order_id' => $order_id))) {
