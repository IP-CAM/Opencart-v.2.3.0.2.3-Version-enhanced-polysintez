	...
	public function addTransaction($affiliate_id, $description = '', $amount = '', $order_id = 0) {
		...
		if ($affiliate_info) {
			...
			$message .= sprintf($this->language->get('text_transaction_total'), $this->currency->format($this->getTransactionTotal($affiliate_id), $this->config->get('config_currency')));
                ...
                //Send Mail to Admin
                $this->model_extension_module_shoputils_mail_order_status->affiliateTransactionAdmin(array('affiliate_id' => $affiliate_id, 'description' => $description, 'amount' => $this->currency->format($amount, $this->config->get('config_currency')), 'total' => $this->currency->format($this->getTransactionTotal($affiliate_id), $this->config->get('config_currency')), 'order_id' => $order_id));
                //Send Mail to Affiliate
                if (!$this->model_extension_module_shoputils_mail_order_status->affiliateTransaction(array('affiliate_id' => $affiliate_id, 'description' => $description, 'amount' => $this->currency->format($amount, $this->config->get('config_currency')), 'total' => $this->currency->format($this->getTransactionTotal($affiliate_id), $this->config->get('config_currency')), 'order_id' => $order_id))) {
