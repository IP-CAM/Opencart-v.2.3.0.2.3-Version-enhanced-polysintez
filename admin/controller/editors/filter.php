<?php
class ControllerEditorsFilter extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$this->document->addScript('view/javascript/admin_quick_edit/filter.js');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply']) && $this->request->post['apply'] == 1) {
				...
				$this->response->redirect($this->url->link('editors/filter/edit', 'token=' . $this->session->data['token'] . '&filter_group_id=' . $filter_group_id . $url, true));
            }

			$this->response->redirect($this->url->link('editors/filter', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply']) && $this->request->post['apply'] == 1) {
				...
				$this->response->redirect($this->url->link('editors/filter/edit', 'token=' . $this->session->data['token'] . '&filter_group_id=' . $this->request->get['filter_group_id'] . $url, true));
            }
	...
	public function copy() {
		...
		if (isset($this->request->post['selected']) && $this->validateCopy()) {
			foreach ($this->request->post['selected'] as $filter_group_id) {
				$new_filter_group_id = $this->model_catalog_filter->copyFilter($filter_group_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('editors/filter', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('editors/filter/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['copy'] = $this->url->link('editors/filter/copy', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('editors/filter/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['filters'][] = array(
				...
				'edit'            => $this->url->link('editors/filter/edit', 'token=' . $this->session->data['token'] . '&filter_group_id=' . $result['filter_group_id'] . $url, true)
			);
		}
		...
		$data['text_filter_value'] = $this->language->get('text_filter_value');	
		...
		$data['filter_name'] = $this->config->get('admin_quick_edit_filter_name');
		$data['filter_sort_order'] = $this->config->get('admin_quick_edit_filter_sortorder');
		$data['filter_value'] = $this->config->get('admin_quick_edit_filter_value');
		...
		$data['sort_name'] = $this->url->link('editors/filter', 'token=' . $this->session->data['token'] . '&sort=fgd.name' . $url, true);
		$data['sort_sort_order'] = $this->url->link('editors/filter', 'token=' . $this->session->data['token'] . '&sort=fg.sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('editors/filter', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('editors/filter/filter_list', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['filter_group_id'])) {
			$data['action'] = $this->url->link('editors/filter/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('editors/filter/edit', 'token=' . $this->session->data['token'] . '&filter_group_id=' . $this->request->get['filter_group_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('editors/filter', 'token=' . $this->session->data['token'] . $url, true);
		...
		$this->response->setOutput($this->load->view('editors/filter/filter_form', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'editors/filter')) {
