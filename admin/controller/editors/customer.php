<?php
class ControllerEditorsCustomer extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('editors/customer');
		...
		$this->document->addScript('view/javascript/admin_quick_edit/customer.js');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply']) && $this->request->post['apply'] == 1) {
				...
				$this->response->redirect($this->url->link('editors/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $customer_id . $url, true));
            }

			$this->response->redirect($this->url->link('editors/customer', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['apply']) && $this->request->post['apply'] == 1) {
				...
				$this->response->redirect($this->url->link('editors/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . $url, true));
            }
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('editors/customer', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('editors/customer/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('editors/customer/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			if ($login_info && $login_info['total'] >= $this->config->get('config_login_attempts')) {
				$unlock = $this->url->link('editors/customer/unlock', 'token=' . $this->session->data['token'] . '&email=' . $result['email'] . $url, true);
			} else {
				$unlock = '';
			}
			...
			$data['customers'][] = array(
				...
				'newsletter'        => $result['newsletter'] ? $this->language->get('text_on') : $this->language->get('text_off'),
				'no_newsletter'     => $result['newsletter'] ? '<span class="text-success">' . $this->language->get('text_enabled') . '</span>' : '<span class="text-danger">' . $this->language->get('text_disabled') . '</span>',
				'approved'          => $result['approved'] ? $this->language->get('text_on') : $this->language->get('text_off'),
				'no_approved'       => $result['approved'] ? '<span class="text-success">' . $this->language->get('text_enabled') . '</span>' : '<span class="text-danger">' . $this->language->get('text_disabled') . '</span>',
				'ip'             	=> $result['ip'],
				'date_added'     	=> date($this->language->get('date_format_short'), strtotime($result['date_added'])),
				'unlock'         	=> $unlock,
				'edit'           	=> $this->url->link('editors/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] . $url, true)
			);
		}
		...
		$data['activated'] = $this->config->get('admin_quick_edit_activated');
		$data['customer_setting_manager'] = $this->config->get('customer_setting_manager');
		$data['customer_name'] = $this->config->get('admin_quick_edit_customer_name');
		$data['customer_email'] = $this->config->get('admin_quick_edit_customer_email');
		$data['customer_edit_group'] = $this->config->get('admin_quick_edit_customer_edit_group');
		$data['customer_status'] = $this->config->get('admin_quick_edit_customer_status');
		$data['customer_newsletter'] = $this->config->get('admin_quick_edit_customer_rss');
		$data['customer_approved'] = $this->config->get('admin_quick_edit_customer_approved');
		$data['customer_telephone'] = $this->config->get('admin_quick_edit_customer_phone');
		$data['customer_password'] = $this->config->get('admin_quick_edit_customer_password');
		$data['customer_field'] = $this->config->get('admin_quick_edit_customer_field');
		$data['customer_address'] = $this->config->get('admin_quick_edit_customer_address');
		$data['customer_history'] = $this->config->get('admin_quick_edit_customer_history');
		$data['customer_transaction'] = $this->config->get('admin_quick_edit_customer_transaction');
		$data['customer_reward'] = $this->config->get('admin_quick_edit_customer_reward');
		
		if ($data['customer_telephone'] || $data['customer_password'] || $data['customer_field'] || $data['customer_address'] || $data['customer_history'] || $data['customer_transaction'] || $data['customer_reward']) {
			$data['customer_more'] = true;
		} else {
			$data['customer_more'] = false;
		}
		...
		$data['sort_name'] = $this->url->link('editors/customer', 'token=' . $this->session->data['token'] . '&sort=name' . $url, true);
		$data['sort_email'] = $this->url->link('editors/customer', 'token=' . $this->session->data['token'] . '&sort=c.email' . $url, true);
		$data['sort_customer_group'] = $this->url->link('editors/customer', 'token=' . $this->session->data['token'] . '&sort=customer_group' . $url, true);
		$data['sort_status'] = $this->url->link('editors/customer', 'token=' . $this->session->data['token'] . '&sort=c.status' . $url, true);
		$data['sort_newsletter'] = $this->url->link('editors/customer', 'token=' . $this->session->data['token'] . '&sort=c.newsletter' . $url, true);
		$data['sort_approved'] = $this->url->link('editors/customer', 'token=' . $this->session->data['token'] . '&sort=c.approved' . $url, true);																		 
		$data['sort_ip'] = $this->url->link('editors/customer', 'token=' . $this->session->data['token'] . '&sort=c.ip' . $url, true);
		$data['sort_date_added'] = $this->url->link('editors/customer', 'token=' . $this->session->data['token'] . '&sort=c.date_added' . $url, true);
		...
		$pagination->url = $this->url->link('editors/customer', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$data['permission'] = $this->user->hasPermission('modify', 'editors/customer');

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('editors/customer/customer_list', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['customer_id'])) {
			$data['action'] = $this->url->link('editors/customer/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('editors/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('editors/customer', 'token=' . $this->session->data['token'] . $url, true);
		...
		$this->response->setOutput($this->load->view('editors/customer/customer_form', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'editors/customer')) {
	...
	public function history() {
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('editors/customer/history', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($history_total) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($history_total - 10)) ? $history_total : ((($page - 1) * 10) + 10), $history_total, ceil($history_total / 10));

		$this->response->setOutput($this->load->view('editors/customer/customer_history', $data));
	}
	...
	public function transaction() {
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('editors/customer/transaction', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($transaction_total) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($transaction_total - 10)) ? $transaction_total : ((($page - 1) * 10) + 10), $transaction_total, ceil($transaction_total / 10));

		$this->response->setOutput($this->load->view('editors/customer/customer_transaction', $data));
	}
	...
	public function reward() {
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('editors/customer/reward', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('editors/customer/customer_reward', $data));
	}
	...
	public function ip() {
		...
		foreach ($results as $result) {
			$data['ips'][] = array(
				...
				'filter_ip'  => $this->url->link('editors/customer', 'token=' . $this->session->data['token'] . '&filter_ip=' . $result['ip'], true)
			);
		}
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('editors/customer/ip', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'] . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('editors/customer/customer_ip', $data));
	}
