...
<div class="rev_slider">
    ...
    <div class="row product-layout">
        <div id="slider_feaut_products" class="owl-carousel">
			...
			<div class="caption product-info clearfix">
				...
				<div class="description_options">
					...
					<?php if ($product['options'] && $product['options_buy']) { ?>
						<div class="well well-sm product-info options_buy">
							<div id="option_<?php echo $product['product_id']; ?>" class="options">
								<div class="form-group">
									...
									<?php } ?>
									<select class="form-control" onchange="get_product_option_id_slider_feaut_products('<?php echo $product['product_id']; ?>')" name="" id="slider_feaut_products_soption_<?php echo $product['product_id']; ?>">
						...
						$('#slider_feaut_products select.form-control option:nth-child(1)').attr('selected', 'selected');
						$( document ).ready(function() {
							get_product_option_id_slider_feaut_products (<?php echo $product['product_id']; ?>);
						});
						function get_product_option_id_slider_feaut_products (product_id) {
							var sel_option_id = document.getElementById('slider_feaut_products_soption_'+product_id);
					...
					<?php } else if ($product['options']) { ?>
						...
						<?php foreach ($product['options'] as $option) { ?>
							...
							<div class="form-group<?php echo ($option['required'] ? ' required' : ''); ?>">
							  <label class="control-label" for="slider_feaut_products_input-option<?php echo $option['product_option_id']; ?>"><?php echo $option['name']; ?>:</label>
							  <select onchange="update_prices_product_viewed_products(<?php echo $product['product_id']; ?>,<?php echo $product['minimum']; ?>);" name="option[<?php echo $option['product_option_id']; ?>]" id="slider_feaut_products_input-option<?php echo $option['product_option_id']; ?>" class="form-control">
							...
							<div class="form-group<?php echo ($option['required'] ? ' required' : ''); ?>">
							  ...
							  <div id="input-option<?php echo $option['product_option_id']; ?>">
								<?php foreach ($option['product_option_value'] as $option_value) { ?>
								<div class="radio">
									<input onchange="update_prices_product_viewed_products(<?php echo $product['product_id']; ?>,<?php echo $product['minimum']; ?>);" type="radio" name="option[<?php echo $option['product_option_id']; ?>]" value="<?php echo $option_value['product_option_value_id']; ?>" id="slider_feaut_products_<?php echo $option['product_option_id']; ?>_<?php echo $option_value['product_option_value_id']; ?>" />
									<label class="btn btn-default <?php if ($option_value['image']) { ?>label-in-img<?php } ?>" for="slider_feaut_products_<?php echo $option['product_option_id']; ?>_<?php echo $option_value['product_option_value_id']; ?>">
							...
							<?php if ($option['type'] == 'image') { ?>
								<div class="form-group<?php echo ($option['required'] ? ' required' : ''); ?>">
								  ...
								  <div id="input-option<?php echo $option['product_option_id']; ?>">
									...
									<div class="radio">
										<input onchange="update_prices_product_viewed_products(<?php echo $product['product_id']; ?>,<?php echo $product['minimum']; ?>);" type="radio" name="option[<?php echo $option['product_option_id']; ?>]" value="<?php echo $option_value['product_option_value_id']; ?>" id="slider_feaut_products_<?php echo $option['product_option_id']; ?>_<?php echo $option_value['product_option_value_id']; ?>" />
										<label <?php if (strstr($option['name'], 'Цвет')) { ?>data-toggle="tooltip" title="<?php echo $option_value['name']; ?>"<?php } ?> class="btn btn-default label-in-img" for="slider_feaut_products_<?php echo $option['product_option_id']; ?>_<?php echo $option_value['product_option_value_id']; ?>">
										<img src="<?php echo $option_value['image']; ?>" alt="<?php echo $option_value['name'] . ($option_value['price'] ? ' ' . $option_value['price_prefix'] . $option_value['price'] : ''); ?>" class="img-thumbnail" /> <?php if (!strstr($option['name'], 'Цвет')) { ?><?php echo $option_value['name']; ?><?php } ?>
										<?php if ($option_value['price']) { ?>
										<span class="option_price"><?php echo $option_value['price_prefix']; ?><?php echo $option_value['price']; ?></span>
										<?php } ?>
...
<script type="text/javascript"><!--
$("#slider_feaut_products").owlCarousel({
	responsiveBaseWidth: '#slider_feaut_products',
	<?php if ($chislo_ryad) { ?>
...
<?php if ($img_slider) { ?>
	$('#slider_feaut_products .owlproduct').owlCarousel({
		items: 1,
		singleItem: true,
		mouseDrag: false,
		touchDrag: false,
...
function get_revpopup_cart_quantity_viewed_products(product_id) {
	input_val = $('#slider_feaut_products .product_'+product_id+' .plus-minus').val();
...
function validate_pole_viewed_products(val, product_id, znak, minimumvalue, maximumvalue) {
	...
	maximumvalue = Number($('#slider_feaut_products .pr_quantity_'+product_id).text());
	if (maximumvalue == 0) maximumvalue = 9999;
	input_val = $('#slider_feaut_products .product_'+product_id+' .plus-minus');	
...
function update_quantity_viewed_products(product_id, quantity) {
	...
	data = $('#slider_feaut_products .product_'+product_id+' .options input[type=\'text\'], #slider_feaut_products .product_'+product_id+' .options input[type=\'hidden\'], #slider_feaut_products .product_'+product_id+' .options input[type=\'radio\']:checked, #slider_feaut_products .product_'+product_id+' .options input[type=\'checkbox\']:checked, #slider_feaut_products .product_'+product_id+' .options select');
	$.ajax({
	  ...
	  success: function(json) {
		<?php if ($recalc_price) { ?>
			...
			var start_weight = parseFloat($('#slider_feaut_products .pr_weight_'+product_id).attr('data-weight'));
			var weight = json['weight'];
			$({val:start_weight}).animate({val:weight}, {
				duration: 500,
				easing: 'swing',
				step: function(val) {
					$('#slider_feaut_products .pr_weight_'+product_id).html(weight_format(val, product_id));
				}
			});
			$('#slider_feaut_products .pr_weight_'+product_id).attr('data-weight', json['weight']);
			<?php } ?>

			<?php if ($stikers_status) { ?>
				var start_price = parseFloat($('#slider_feaut_products .special_no_format'+product_id).html().replace(/\s*/g,''));
				var price = json['price_n'];
				$({val:start_price}).animate({val:price}, {
					duration: 500,
					easing: 'swing',
					step: function(val) {
						$('#slider_feaut_products .special_no_format'+product_id).html(price_format(val));
					}
				});
			<?php } ?>
			
			var start_special = parseFloat($('#slider_feaut_products .price_no_format'+product_id).html().replace(/\s*/g,''));
			var special = json['special_n'];
			$({val:start_special}).animate({val:special}, {
				duration: 500,
				easing: 'swing',
				step: function(val) {
					$('#slider_feaut_products .price_no_format'+product_id).html(price_format(val));
				}
			});
		<?php } ?>
	  }
...
function update_prices_product_viewed_products(product_id, minimumvalue, block_id) {
	...
	if (input_val > minimumvalue) {
		...
		$('#slider_feaut_products .product_'+product_id+' .plus-minus').val(minimumvalue);
	}
	<?php if ($recalc_price) { ?>
	quantity = parseInt(input_val);
	<?php } else { ?>
	...
	$.ajax({
	  ...
	  success: function(json) {
		...
		<?php if ($img_slider) { ?>
		$('#slider_feaut_products .product_'+product_id+' .image .owl-item:first-child img').attr('src', json['opt_image']);
		<?php } else { ?>
		$('#slider_feaut_products .product_'+product_id+' .image img').attr('src', json['opt_image']);
		<?php } ?>
		
		<?php if ($description_options['zamena_description'] && $description_options['model']) { ?>
		$('#slider_feaut_products .product_'+product_id+' .pr_model_'+product_id).html(json['opt_model']);
		<?php } ?>
	  
		var start_quantity = parseFloat($('#slider_feaut_products .pr_quantity_'+product_id).html().replace(/\s*/g,''));
		var end_quantity = json['option_quantity'];
		$({val:start_quantity}).animate({val:end_quantity}, {
			duration: 500,
			easing: 'swing',
			step: function(val) {
				$('#slider_feaut_products .pr_quantity_'+product_id).html(number_format(val, product_id));
			}
		});
...
max_height_div('#slider_feaut_products .product-thumb h4');
$('#slider_feaut_products .product-thumb .description_options').css('min-height', 'initial');
max_height_div('#slider_feaut_products .product-thumb .description');
max_height_div('#slider_feaut_products .product-thumb .description_options');
max_height_div('#slider_feaut_products .product-thumb .price');
max_height_div('#slider_feaut_products .product-thumb .number');
