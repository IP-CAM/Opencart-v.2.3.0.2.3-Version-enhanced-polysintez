			...
			<div class="caption product-info clearfix">
				...
				<div class="description_options">
					...
					<?php if ($product['options'] && $product['options_buy']) { ?>
						<div class="well well-sm product-info options_buy">
							<div id="option_<?php echo $product['product_id']; ?>" class="options">
								<div class="form-group">
									...
									<?php } ?>
									<select class="form-control" onchange="get_product_option_id_slider_lastc_products('<?php echo $product['product_id']; ?>')" name="" id="slider_lastc_products_soption_<?php echo $product['product_id']; ?>">
						...
						$('#slider_lastc_products select.form-control option:nth-child(1)').attr('selected', 'selected');
						$( document ).ready(function() {
							get_product_option_id_slider_lastc_products (<?php echo $product['product_id']; ?>);
						});
						function get_product_option_id_slider_lastc_products (product_id) {
							var sel_option_id = document.getElementById('slider_lastc_products_soption_'+product_id);
					...
					<?php } else if ($product['options']) { ?>
						...
						<?php foreach ($product['options'] as $option) { ?>
							...
							<div class="form-group<?php echo ($option['required'] ? ' required' : ''); ?>">
							  <label class="control-label" for="slider_lastc_products_input-option<?php echo $option['product_option_id']; ?>"><?php echo $option['name']; ?>:</label>
							  <select onchange="update_prices_product_viewed_products(<?php echo $product['product_id']; ?>,<?php echo $product['minimum']; ?>);" name="option[<?php echo $option['product_option_id']; ?>]" id="slider_lastc_products_input-option<?php echo $option['product_option_id']; ?>" class="form-control">
							...
							<div class="form-group<?php echo ($option['required'] ? ' required' : ''); ?>">
							  ...
							  <div id="input-option<?php echo $option['product_option_id']; ?>">
								<?php foreach ($option['product_option_value'] as $option_value) { ?>
								<div class="radio">
									<input onchange="update_prices_product_viewed_products(<?php echo $product['product_id']; ?>,<?php echo $product['minimum']; ?>);" type="radio" name="option[<?php echo $option['product_option_id']; ?>]" value="<?php echo $option_value['product_option_value_id']; ?>" id="slider_lastc_products_<?php echo $option['product_option_id']; ?>_<?php echo $option_value['product_option_value_id']; ?>" />
									<label class="btn btn-default <?php if ($option_value['image']) { ?>label-in-img<?php } ?>" for="slider_lastc_products_<?php echo $option['product_option_id']; ?>_<?php echo $option_value['product_option_value_id']; ?>">
							...
							<?php if ($option['type'] == 'image') { ?>
								<div class="form-group<?php echo ($option['required'] ? ' required' : ''); ?>">
								  ...
								  <div id="input-option<?php echo $option['product_option_id']; ?>">
									...
									<div class="radio">
										<input onchange="update_prices_product_viewed_products(<?php echo $product['product_id']; ?>,<?php echo $product['minimum']; ?>);" type="radio" name="option[<?php echo $option['product_option_id']; ?>]" value="<?php echo $option_value['product_option_value_id']; ?>" id="slider_lastc_products_<?php echo $option['product_option_id']; ?>_<?php echo $option_value['product_option_value_id']; ?>" />
										<label <?php if (strstr($option['name'], 'Цвет')) { ?>data-toggle="tooltip" title="<?php echo $option_value['name']; ?>"<?php } ?> class="btn btn-default label-in-img" for="slider_lastc_products_<?php echo $option['product_option_id']; ?>_<?php echo $option_value['product_option_value_id']; ?>">
										<img src="<?php echo $option_value['image']; ?>" alt="<?php echo $option_value['name'] . ($option_value['price'] ? ' ' . $option_value['price_prefix'] . $option_value['price'] : ''); ?>" class="img-thumbnail" /> <?php if (!strstr($option['name'], 'Цвет')) { ?><?php echo $option_value['name']; ?><?php } ?>
										<?php if ($option_value['price']) { ?>
										<span class="option_price"><?php echo $option_value['price_prefix']; ?><?php echo $option_value['price']; ?></span>
										<?php } ?>
				...
				<div class="product_buttons">
					...
					<?php if ($product['quantity'] > 0 || $zakaz) { ?>
						<?php if ($product['price_number'] > 0 || $zakaz_price_null) { ?>
							<div class="cart">
								<a onclick="get_revpopup_cart(<?php echo $product['product_id']; ?>,'module',get_revpopup_cart_quantity_viewed_products('<?php echo $product['product_id']; ?>'),'slider_lastc_products');" <?php if ($button_cart_class != 'prlistb active') { ?>data-toggle="tooltip" title="<?php echo $button_cart; ?>"<?php } ?>><i class="fa fa-border fa-shopping-basket"><span class="<?php echo $button_cart_class; ?>"><?php echo $button_cart; ?></span></i></a>
							</div>
						<?php } ?>
					<?php } ?>
				</div>
	...
	$.ajax({
	  ...
	  success: function(json) {
		<?php if ($recalc_price) { ?>
			...
			var start_weight = parseFloat($('#slider_lastc_products .pr_weight_'+product_id).attr('data-weight'));
			var weight = json['weight'];
			$({val:start_weight}).animate({val:weight}, {
				duration: 500,
				easing: 'swing',
				step: function(val) {
					$('#slider_lastc_products .pr_weight_'+product_id).html(weight_format(val, product_id));
				}
			});
			$('#slider_lastc_products .pr_weight_'+product_id).attr('data-weight', json['weight']);
			<?php } ?>
...
max_height_div('#slider_lastc_products .product-thumb .description');
