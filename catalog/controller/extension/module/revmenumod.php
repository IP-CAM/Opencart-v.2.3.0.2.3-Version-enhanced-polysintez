...
class ControllerExtensionModuleRevmenumod extends Controller {
	public function index() {
		...
		$setting = $this->config->get('revmenumod');
		...
		if (VERSION >= 2.2) {
			$cache_data = $this->cache->get('revmenu.' . (int)$this->config->get('config_language') . '.' . (int)$this->config->get('config_store'));
		} else {
		...
		if ($cache_data) {
			...
			if ($setting_all_settings['cache_on']) {
				if (VERSION >= 2.2) {
					$this->cache->set('revmenu.' . (int)$this->config->get('config_language') . '.' . (int)$this->config->get('config_store'), $data['categories']);
				} else {
